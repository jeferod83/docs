...
As configurações so geralmente armazenadas tanto no PHP como em arquivos INI, e
so carregadas durante o "bootstrap" da aplicação. O CakePHP vem com um único arquivo
de configuração por padrão, mas, se necessário, você pode acrescenar arquivos de
...
Se sua aplicação tem muitas opções de configuração, pode ser útil dividir elas
em vários arquivos. Após criá-los dentro do diretório **config/**, você pode
carregá-los no **bootstrap.php**::
...
Você pode também utilizar arquivos de configuração adicionais para criar variáveis
específicas para cada ambiente. Cada arquivo carregado após o **app.php** pode
redefinir valores criados anteriormente, permitindo que você customize configurações
específicas para um ambiente de desenvolvimento ou homologação.

Configuração Geral
------------------

Abaixo está uma descrição das variáveis e como elas afetam o comportamento da sua
aplicação CakePHP

debug
    Modifica a saída de depuração do CakePHP. ``false`` = Modo de Produção. Sem
    mensagens de erros, erros ou avisos so mostrados. ``true`` = Erros e avisos são
    mostrados.
App.namespace
    O "namespace" que irá conter as classes.

    .. note::

        Quando for modificado o "namespace" nas configuraçes, será necessário
        atualizar o arquivo **composer.json** para utilizar este "namespace".
        Adicionalmente, crie um novo "autoloader" executando
